...
export const Pagination = ({ pageUrl, setCurrentPage, page, posts }) => {
    ...
    return (
        <Wrapper>
            <Content>
                <Left onClick={e => { setCurrentPage(page === 1 ? 1 : page - 1) }} to={page < 3 ? pageUrl : pageUrl + '?page=' + (page - 1)} >
                ...
                <Right onClick={e => { setCurrentPage(page === pagesCount ? pagesCount : page + 1) }} to={page > pagesCount - 2 ? pageUrl + '?page=' + pagesCount : pageUrl + '?page=' + (page + 1)} >
                    <svg xmlns="http://www.w3.org/2000/svg" width="5.434" height="5.746" viewBox="0 0 5.434 5.746">
                        <path id="Path_132" data-name="Path 132" d="M.819-1.677h2.8L6.253-4.55,3.614-7.423H.819L3.471-4.55Z" transform="translate(-0.819 7.423)" fill="#141c2b" />
                    </svg>
                </Right>
