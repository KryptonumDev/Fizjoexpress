        ...
        <Content>
          <Slider ref={slickRef} {...settings}>
            {opinieKlientow.map((el) => (
              <Item
                className='reviews-slider-items'
                key={`${el.nazwa}-${el.data}`}>
                <svg
                  className='quote'
                ...
                <div
                  ...
                  dangerouslySetInnerHTML={{
                    __html: el.trescOpinii
                  }}
                />
                <div className='flex'>
                  <span className='text'>{el.nazwa}</span>
                  <span className='text'>{el.data}</span>
                </div>
              </Item>
            ))}
          </Slider>
...
const Item = styled.div`
  ...
  padding: 85px clamp(20px, ${(40 / 1366) * 100}vw, 40px)
    clamp(30px, ${(50 / 1366) * 100}vw, 50px)
    clamp(20px, ${(40 / 1366) * 100}vw, 40px);
  position: relative;
